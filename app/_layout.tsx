...
function LayoutContent() {
    ...
    const shouldSkipOuterScroll = isLoginPage || isDontExercisePage || isExercisePage || isWorkoutPage;
    const shouldShowBackArrow = !isLoginPage && !isDontExercisePage && !isHomePage && !isExercisePage;
    ...
    return (
        <>
            <View
                style={{ flex: 1, flexDirection: Platform.OS === "web" ? "row" : "column", backgroundColor: "#F9FAFB" }}
            >
                <Header />
                {Platform.OS !== "web" && shouldShowBackArrow && <BottomNav />}
                {Platform.OS === "web" && !isLoginPage && <Sidebar />}
                {!shouldSkipOuterScroll ? (
